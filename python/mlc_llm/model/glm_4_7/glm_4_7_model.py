"""Implementation for the GLM-4.7 Flash architecture."""
...
class GLM47FlashConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """Configuration of the GLM-4.7 Flash model."""
    ...
    def __post_init__(self):
        ...
        if self.head_dim == 0:
            if self.qk_nope_head_dim > 0 and self.qk_rope_head_dim > 0:
...
class GLM47FlashAttention(nn.Module):  # pylint: disable=too-many-instance-attributes
    """Multi-head attention for GLM-4.7 Flash with partial RoPE support."""
...
class GLM47FlashDenseMLP(nn.Module):
...
class GLM47FlashSparseMoeBlock(nn.Module):  # pylint: disable=too-many-instance-attributes
    """Sparse MoE layer for GLM-4.7 Flash."""
    ...
    def forward(self, x: Tensor):
        ...
        # Use configurable top-k routing.
...
class GLM47FlashDecoderLayer(nn.Module):
    """Decoder layer with hybrid dense/MoE support."""

    def __init__(self, config: GLM47FlashConfig, layer_idx: int):
        super().__init__()
        self.layer_idx = layer_idx
        self.self_attn = GLM47FlashAttention(config)
        ...
        if layer_idx < config.first_k_dense_replace:
            self.mlp = GLM47FlashDenseMLP(config)
            self.is_moe = False
        else:
            self.mlp = GLM47FlashSparseMoeBlock(config)
            self.is_moe = True

        self.tensor_parallel_shards = config.tensor_parallel_shards
        self._set_tp(config)

    def _set_tp(self, config: GLM47FlashConfig):
        def _set(layer, hint):
...
class GLM47FlashModel(nn.Module):
    """GLM-4.7 Flash transformer model."""

    def __init__(self, config: GLM47FlashConfig):
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList(
            [GLM47FlashDecoderLayer(config, i) for i in range(config.num_hidden_layers)]
        )
...
class GLM47FlashForCausalLM(nn.Module):  # pylint: disable=too-many-instance-attributes
    """GLM-4.7 Flash model for causal language modeling."""

    def __init__(self, config: GLM47FlashConfig):
        self.model = GLM47FlashModel(config)

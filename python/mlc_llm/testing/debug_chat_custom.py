...
class DebugChat:  # pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def _preprocess_prompts(
        ...
        print(f"{green('Input tokens from conversation template')}: {tokens}")
        tokens = [
            [
                200022,
                3575,
        ...
        print(f"{red('Hardcoded input tokens from HF Tokenize for debugging')}: {tokens}")
        return tokens
    ...
    def generate(
        ...
        exit("Stopping after Prefill for debugging...")
